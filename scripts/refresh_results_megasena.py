...
cursor = conn.cursor()
cursor.execute("""
CREATE TABLE IF NOT EXISTS concursos_megasena (
...
cursor.execute("SELECT COALESCE(MAX(concurso), 0) FROM concursos_megasena")
ultimo_salvo = cursor.fetchone()[0]
print("Último concurso salvo:", ultimo_salvo)

# Último concurso da API
API_ULTIMO = "https://api.guidi.dev.br/loteria/megasena/ultimo"
...
def baixar_concurso(concurso):
    try:
        url = f"https://api.guidi.dev.br/loteria/megasena/{concurso}"
...
def carregar_historico():
    """Retorna todas as dezenas da Mega-Sena como lista de sets"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute(
        "SELECT dezena1, dezena2, dezena3, dezena4, dezena5, dezena6 FROM concursos_megasena ORDER BY concurso ASC"
    )
...
if novos_concursos:
    cursor.executemany("""
    INSERT OR IGNORE INTO concursos_megasena
